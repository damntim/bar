...
function updateCartDisplay() {
    ...
    cart.forEach(item => {
        ...
        const sanitizedSubtotal = Number(item.subtotal).toFixed(2);
        const sanitizedQuantity = Number(item.quantity);

        itemElement.innerHTML = `
            <div>
                <h4 class="font-medium text-gray-800">${sanitizedName}</h4>
                <div class="text-sm text-indigo-600">$${sanitizedPrice} × ${sanitizedQuantity} = $${sanitizedSubtotal}</div>
            </div>
            <div class="flex items-center gap-2">
                <button 
                    onclick="updateQuantity(${item.id}, -1)"
                    class="px-2 py-1 bg-indigo-100 hover:bg-indigo-200 rounded text-indigo-600 ${sanitizedQuantity <= 1 ? 'opacity-50 cursor-not-allowed' : ''}"
                    aria-label="Decrease quantity"
                    ${sanitizedQuantity <= 1 ? 'disabled' : ''}
                >-</button>
                <span class="text-gray-800 min-w-[20px] text-center">${sanitizedQuantity}</span>
                <button 
                    onclick="updateQuantity(${item.id}, 1)"
...
function showNotification(message) {
    ...
    notification.className = 'fixed top-4 right-4 bg-indigo-600 text-white px-4 py-2 rounded shadow-lg';
